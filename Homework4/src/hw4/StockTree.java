...
public class StockTree<T extends Food>
						extends Tree<T>
						implements IBST<T> 
{
	...
	private T findEntry(BinaryNode<T> rootNode, T entry) {
		...
		if (rootNode != null)
		{
			...
			if(entry.equals(rootEntry))//same instance
				result = rootEntry;
			else if (entry.compareTo(rootEntry.getStock()) == 0)//same rating
				result = rootEntry;
			else if (entry.compareTo(rootEntry.getStock()) < 0)
	...
	@Override
	public T add(T entry) {//returns null if entry doesn't exist in tree
		T result = null;
	...
	private T addEntry(BinaryNode<T> rootNode, T newEntry) {
		...
		T result = null;
		int comparison = newEntry.compareTo(rootNode.getData().getStock());
	...
	private BinaryNode<T> removeEntry(BinaryNode<T> rootNode, T entry, ReturnObject oldEntry) {
		if (rootNode != null)
		{
			...
			int comparison = entry.compareTo(rootData.getStock());
			if (comparison == 0)
			{
